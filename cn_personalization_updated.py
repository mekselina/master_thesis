...
print(items.head())
print(items.index)
out_df = items[672:].copy()
personalized_cns = []
for i, row in items[672:].iterrows():
    out_current = items.iloc[[i]].copy()
    cn = row['cn']
    age = 'older' if row['age'] == 25 else 'younger'
    gender = 'man ' if row['gender'] == 'm' else 'woman '
    demo = 'for a ' + gender + age
    personalized_cn = get_personalized_cn(cn, demo, instructions, cn_prefix, demo_prefix, model_name="text-davinci-003", use_example=True, example=example)
    ...
    out_current['personalized_cn'] = personalized_cn
    out_current.to_csv(f"cn_generations/personalized_cn_cont2_{i}.csv", sep=';', index=False)

    sleep(10)

out_df['personalized_cn'] = personalized_cns

out_df.to_csv("personalized_cn_allp.csv", sep=';', index=False)
